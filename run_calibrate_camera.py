...
def main():
    ...
    # In this situation, we use gstreamer backend for the camera capture.
    ...
    cap_thread = CaptureThread(device_id=device,
                               ...
                               resolution=(W, H),
                               use_gst=not args.no_gst, # 传入值为真
                               )
    buffer_manager = MultiBufferManager()
    # In this step add the device to the manager buffer.
    buffer_manager.bind_thread(cap_thread, buffer_size=8)
    # Test if the camera can be connected correctly.
    if cap_thread.connect_camera(): # connect to camera
        cap_thread.start() # 翻转180度
    else:
    ...
    while True:
        ...
        # retval, corners	=	cv.findChessboardCorners(	image, patternSize[, corners[, flags]]	)  # 调用接口
        ...
        if found:
            ...
            # Refines the corner locations.  	corners	=	cv.cornerSubPix(	image, corners, winSize, zeroZone, criteria	)  
            # term is the criteria of the algorithm stop condition.
